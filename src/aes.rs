...
lazy_static!{
    ...
    /// ];
    /// for (i, &n) in sbox.iter().enumerate() {
    ///     assert_eq!(SBOX[i], n);
    /// }
    /// ```
    ...
    /// ];
    /// for (i, &n) in rsbox.iter().enumerate() {
    ///     assert_eq!(RSBOX[i], n);
    /// }
    /// ```
    ...
    pub static ref RCON: [u8; 256] = {
        ...
        let mut rconbox = [0; 256];
        for (i, n) in rconbox.iter_mut().enumerate() {
            *n = rcon(i as u8);
        }
        rconbox
    };
